...
// 전개연산자 구문!!!
// 전개연산자 구문이 함수의 매개변수에서 사용되면 나머지 매개변수라고 특정한 명칭을 갖게됨
...
// 심벙을 생성하는 또 다른 방법 => 전역 심볼 레지스트리에 저장하는 방법
// 아무리 유일무이한 심볼값이 장점이라고 하지만, 심볼간 값을 공유하거나 논리적으로 비교해야 하는 상황이 발생되는 경우에는 전역 심볼 레지스트리에 저장을 하는 것이 좋음!
// Symbol()은 고유한 값을 가지고 있는 심벌을 생성한 후 반환하지만, Symbol.for()는 전역적으로 존재하는 global symbol table(*전역 심볼 레지스트리)에 저장 및 데이터를 찾아오는 역할!!
